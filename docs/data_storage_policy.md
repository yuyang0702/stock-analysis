...
> 2026-07-19 schema 9 增量已随 `5ad0ad539ef66aa7cf1073ad7142fde116d74ea5` 推送并部署服务器：正式交易库新增 `gap_reentry_opportunities`，每只股票、每个交易日、每个机会只插入一行并按状态更新，不保存逐扫描历史；索引只覆盖交易日/状态与股票/交易日。该表纳入现有在线备份、SHA-256、完整性检查、核心表计数和7/4/12轮转，不新增 JSONL、逐轮报告或秘密字段。按最多30只候选×250交易日估算上限约7500行/年，年度容量复核并沿用交易账本长期审计策略。被交易控制拦截的信号不写 `new_signal_id`，避免把未发布机会固化成已发布；订单与成交细节继续通过既有信号、订单和成交表关联，不复制到逐扫描记录。部署前 schema 8 在线备份完整性为 `ok`，迁移后 schema 9健康/可写且环境哈希不变。当前为 `implemented（已推送） / deployed（服务器，功能关闭） / not observed / not validated`。
...
- 按每批30只、每日约48批、每年250个交易日估算约36万候选行；ML库目标低于1GB/年，超过1GB/年告警，超过2GB/年停止新增特征和候选明细但不影响规则交易。`ML_DB_MAX_BYTES` 的硬限制统计主库逻辑容量和 WAL 数据增长；SQLite 固定大小的 SHM 协调文件不计入该数据配额，但仍单独监控和告警。写入必须在主库与 WAL 的预留增长超过上限前拒绝。
- 最近400个自然日保持热数据；更早完整标签按年度只读归档，任何清理必须 dry-run、备份校验和人工授权。
- 人工批准、实际启用和回滚所需模型长期保留；未批准 challenger 保留最近12个或180天；latest报告原子覆盖，每周最多一份常规归档。
- ML库复用SQLite在线备份、7/4/12轮转、完整性检查和季度恢复演练；模型文件使用SHA-256与数据库登记双向校验。
- 运行中的 WAL 数据库只允许使用普通只读、WAL-aware 连接读取，不得使用 `immutable=1`；初始化在首次建库或迁移时预留完整建表空间，已完成同版本初始化时走幂等校验，不重复要求整套建表预留。
- 现行历史回测库权威上限保持为目标低于2GB/年、达到3GB导入上限即拒绝；不得使用4GB或其他宽松口径。
- 任何ML写入、训练或推理失败都只关闭该批数据或模型影响，不直接改变买卖、风控或 `kill_switch`。
