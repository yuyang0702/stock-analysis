...
| ML-3 复盘报表 | 已实现 | 可生成 `output/ml_signal_review.md`，统计样本数、买卖数量、订单状态、原策略分布和影子评分分布；这只是复盘报表，不训练模型、不参与下单。 |
...
| ML-6 影子模型 | 已实现第一版 | 当前不是训练模型，而是规则型影子评分；只输出 `enhanced_score`、`shadow_rank` 和原因，不影响真实下单，用来和原策略做对照。 |
