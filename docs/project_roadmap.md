...
- `shadow_score.py` 会基于原策略分、消息催化、题材热度、板块位置、市场情绪、交易质量和海外风险生成 `enhanced_score`、`shadow_rank` 和 `shadow_reason`，只用于对照复盘。
...
| ML-6 影子模型 | 已实现第一版 | 当前不是训练模型，而是规则型影子评分；已纳入消息催化、题材热度、实时板块位置、市场情绪、交易质量和海外风险，只输出 `enhanced_score`、`shadow_rank` 和原因，不影响真实下单，用来和原策略做对照。 |
