...
模拟盘稳定性账本与策略验证专项设计见：`docs/superpowers/specs/2026-07-11-simulation-stability-ledger-design.md`。Batch 1 当前状态为“代码已实现、待服务器部署和 1 个交易日双写观察”；该文档从属于本文，并进一步细化 SQLite 运行账本、订单幂等、自动对账、观察型风控和 20 个有效交易日验收方案。服务器部署和交易日观察尚未完成，后续状态仍以本文为准。
