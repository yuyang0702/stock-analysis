...
模拟盘稳定性账本与策略验证专项设计见：`docs/superpowers/specs/2026-07-11-simulation-stability-ledger-design.md`。Batch 1 当前状态为“代码已实现并部署服务器、待部署后首个有效交易日双写观察”；2026-07-12 只读核验确认服务器、本地代码均为 `54eaaf423f690dda84776304c4ec87846aa8cf66`，核心服务和定时器已运行，SQLite schema version 为 `1` 且当时 JSON/SQLite 空信号一致。该文档从属于本文，并进一步细化 SQLite 运行账本、订单幂等、自动对账、观察型风控和 20 个有效交易日验收方案。交易日观察尚未完成，后续状态仍以本文为准。
...
新电脑、新Codex对话和跨环境接管入口见：`docs/project_handoff.md`。该文件是可提交的时间点快照和接管清单，不替代本文档；服务器实时状态必须重新验证，私密配置、运行数据、SSH密钥和本机自动化不得通过Git同步。
...
非交易日生成的 readiness 结论只表示静态文件、schema 和配置检查结果，不构成有效观察日、阶段放行或实盘准入证据。当前运行数据治理仅为部分实现：规范已经建立，但健康/API 单文件 JSONL 月度轮转、盘中扫描文件保留、统一归档、备份和恢复演练仍待实施。
