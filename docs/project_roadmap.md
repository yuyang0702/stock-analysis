...
| ML-5 影子模型 | 待实现 | 模型只输出 `ml_score` 和建议，不影响真实下单，用来和原策略做对照。 |
| ML-6 策略辅助 | 待实现 | 只允许机器学习做排序、过滤或小幅仓位调整，止损、单票上限、总仓位上限仍由硬规则控制。 |
| ML-7 自动优化 | 待实现 | 在样本足够后，定期训练、回测、模拟盘对照，只有通过门槛才更新线上参数。 |
...
6. 再实现影子模型，只输出 `ml_score`，不参与真实模拟下单。
7. 当影子模型连续优于原策略后，再考虑让它参与排序、过滤或仓位微调。
