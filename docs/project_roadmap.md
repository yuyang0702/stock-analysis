...
| ML-2 成交与收益标注 | 部分实现 | JoinQuant 回传订单后会补充订单状态、原因、订单号、数量、成交量和价格；多日收益、最大浮盈浮亏等持仓后标签待后续补齐。 |
| ML-3 复盘报表 | 已实现 | 可生成 `output/ml_signal_review.md`，统计样本数、买卖数量、订单状态和分数分布；当前不训练模型、不参与下单。 |
...
| ML-6 影子模型 | 待实现 | 模型只输出 `ml_score` 和建议，不影响真实下单，用来和原策略做对照。 |
