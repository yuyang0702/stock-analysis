...
> 状态：本地 `implemented`，服务器 `not deployed / not observed / not validated`。`trading_backup.py`、7/4/12 轮转、隔离恢复演练、状态报告、告警复用和 systemd 模板已通过本地测试；服务器安装、自动运行证据和季度演练验收仍需单独授权与真实观察。
