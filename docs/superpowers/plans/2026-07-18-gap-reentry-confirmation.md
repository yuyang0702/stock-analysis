...
- [ ] **Step 1: Write failing calculation and state tests**
...
- [ ] **Step 3: Implement immutable inputs and pure evaluator**
...
- [ ] **Step 3: Add schema and minimal APIs**
...
- [ ] **Step 4: Verify migration, health, backup table count and regression**
...
- [ ] **Step 4: Integrate the narrow exception**
...
- [ ] **Step 5: Rebuild the execution plan**
...
- [ ] **Step 6: Run exporter regression and commit**
...
- [ ] **Step 1: Write failing one-lot tests**
...
- Modify: `tests/test_signal_lifecycle.py`
...
Run: `python -m unittest tests.test_signal_lifecycle -v`
Expected: FAIL because the template does not inspect `reentry_cap_price`.

- [ ] **Step 3: Add final checks**
...
Run: `python -m unittest tests.test_signal_lifecycle tests.test_joinquant_health -v`
Expected: PASS.

```bash
git add joinquant_strategy.py tests/test_signal_lifecycle.py tests/test_joinquant_health.py
...
Run: `python -m unittest tests.test_alert_markdown tests.test_joinquant_health tests.test_sqlite_backup -v`
...
```bash
git add a_share_strategy.py joinquant_health.py sqlite_backup.py tests/test_alert_markdown.py tests/test_joinquant_health.py tests/test_sqlite_backup.py
...
python -m unittest tests.test_gap_reentry tests.test_trading_store tests.test_joinquant_exporter tests.test_signal_lifecycle tests.test_alert_markdown tests.test_joinquant_health tests.test_sqlite_backup -v
```
...
Expected: all runnable tests pass with no new failures.

- [ ] **Step 3: Verify storage and Git hygiene**

```bash
python run_ubuntu.py ledger-check
git diff --check
git status --short --branch
```

Use the repository's actual Windows ledger-check entry if `run_ubuntu.py` is unavailable. Do not touch the production database.

- [ ] **Step 4: Update statuses and checkboxes**
