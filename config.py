...
JOINQUANT_TEMPLATE_VERSION = "2026-07-18.1-gap-reentry"
GAP_REENTRY_ENABLE_DEFAULT = _env_bool("GAP_REENTRY_ENABLE", False)
...
# 兼容保留的网页历史操作日志；新面板控制操作写入正式交易账本。
PORTFOLIO_EVENTS_FILE = PORTFOLIO_DIR / "events.jsonl"

# 只读保留旧版截图目录；新面板不再提供上传或访问路由。
...
# 面板认证凭据；为空时兼容复用现有 JoinQuant 同步 token，不改变其值。
PORTFOLIO_WEB_TOKEN = _env_text("PORTFOLIO_WEB_TOKEN", JOINQUANT_SYNC_TOKEN)

# 环境变量名，便于 Linux 用 EnvironmentFile 管理。
ENV_PORTFOLIO_WEB_HOST = "PORTFOLIO_WEB_HOST"
ENV_PORTFOLIO_WEB_PORT = "PORTFOLIO_WEB_PORT"
ENV_PORTFOLIO_WEB_TOKEN = "PORTFOLIO_WEB_TOKEN"
